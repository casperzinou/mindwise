...
import { scrapeWebsite } from '../_shared/scraper.ts' // We will create this shared file
...
Deno.serve(async (req) => {
  try {
    ...
    if (fetchError) throw new Error(`Failed to fetch chatbot: ${fetchError.message}`)

    // 2. Scrape the website
    const scrapedText = await scrapeWebsite(chatbot.website_url)
